...
describe "IMAP Retriever" do
  ...
  describe "handling of options" do
    it "should set default options" do
      ...
      expect(Mail::StringHelper.blank?(options[:count])).not_to be_truthy
      expect(options[:count]).to eq 10

      expect(Mail::StringHelper.blank?(options[:order])).not_to be_truthy
      expect(options[:order]).to eq :asc

      expect(Mail::StringHelper.blank?(options[:what])).not_to be_truthy
      expect(options[:what]).to eq :first

      expect(Mail::StringHelper.blank?(options[:mailbox])).not_to be_truthy
